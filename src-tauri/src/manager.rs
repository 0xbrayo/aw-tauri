...
pub struct ManagerState {
    ...
    pub modules_restart_count: HashMap<String, i32>,
                ...
                if output.status.success() {
                    ...
                    thread::spawn(move || {
                        ...
                        let restart_count = state
                            .modules_restart_count.get(&name_clone).unwrap_or(&0);
                        
                        let pending_shutdown = state
                            .modules_pending_shutdown
                            .get(&name_clone).unwrap_or(&false);
                        let _ = state;
                        if *restart_count < 3 && !*pending_shutdown {
                            let mut state = state_clone.lock().unwrap();
                            state.modules_restart_count.insert(name_clone.clone(), *restart_count + 1);
